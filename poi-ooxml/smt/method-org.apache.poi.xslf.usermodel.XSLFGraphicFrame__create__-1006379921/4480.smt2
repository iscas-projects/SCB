(set-option :produce-unsat-cores true) ; enable generation of unsat cores
(set-option :produce-models true) ; enable model generation
(set-logic ALL)
(declare-sort var1174 0)
(declare-sort var1570 0)
(declare-sort var2669 0)
(declare-sort var1349 0)
(declare-sort var1166 0)
(declare-sort void 0)
(declare-sort Iterator 0)
(declare-sort ClassObject 0)
(declare-fun var2669_getUri/626169278 (var1174) String)
(declare-fun hashCode/-467973558 (String) Int)
(declare-fun var1166-init () var1166)
(declare-fun <init>/2139047500 (var1166 var1174 var1570) void)
(declare-const null-var1174 var1174)
(declare-const null-var1570 var1570)
(declare-const null-String String)
(declare-const var1122 var1174) ; Statement: r0 := @parameter0: org.openxmlformats.schemas.presentationml.x2006.main.CTGraphicalObjectFrame 
(assert (not (= var1122 null-var1174)))
(declare-const var2317 var1570) ; Statement: r3 := @parameter1: org.apache.poi.xslf.usermodel.XSLFSheet 
(assert (not (= var2317 null-var1570)))
(define-const var996 String (var2669_getUri/626169278 var1122)) ; Statement: r1 = staticinvoke <org.apache.poi.xslf.usermodel.XSLFGraphicFrame: java.lang.String getUri(org.openxmlformats.schemas.presentationml.x2006.main.CTGraphicalObjectFrame)>(r0) 
 ; Statement: if r1 != null goto $r7 = r1 
(assert (not (not (= var996 null-String)))) ; Negate: Cond: r1 != null  
(define-const var1336 String "") ; Statement: $r7 = "" 
 ; Statement: goto [?= b1 = -1] 
(assert true) ; Non Conditional
(define-const var1334 Int (- 1)) ; Statement: b1 = -1 
(assert true)
(define-const var111 Int (hashCode/-467973558 var1336)) ; Statement: $i0 = virtualinvoke $r7.<java.lang.String: int hashCode()>() 
 ; Statement: lookupswitch($i0) {     case -844661481: goto $z2 = virtualinvoke $r7.<java.lang.String: boolean equals(java.lang.Object)>("http://schemas.openxmlformats.org/presentationml/2006/ole");     case -809269002: goto $z1 = virtualinvoke $r7.<java.lang.String: boolean equals(java.lang.Object)>("http://schemas.openxmlformats.org/drawingml/2006/diagram");     case -603062095: goto $z0 = virtualinvoke $r7.<java.lang.String: boolean equals(java.lang.Object)>("http://schemas.openxmlformats.org/drawingml/2006/table");     default: goto tableswitch(b1) {     case 0: goto $r5 = new org.apache.poi.xslf.usermodel.XSLFTable;     case 1: goto $r4 = new org.apache.poi.xslf.usermodel.XSLFObjectShape;     case 2: goto $r2 = new org.apache.poi.xslf.usermodel.XSLFDiagram;     default: goto $r6 = new org.apache.poi.xslf.usermodel.XSLFGraphicFrame; }; } 
(assert (and (not (= var111 (- 603062095))) (and (not (= var111 (- 809269002))) (and (not (= var111 (- 844661481))) true)))) ; Intersect: Negate: Cond: $i0 == -603062095   and Intersect: Negate: Cond: $i0 == -809269002   and Intersect: Negate: Cond: $i0 == -844661481   and Non Conditional   
 ; Statement: tableswitch(b1) {     case 0: goto $r5 = new org.apache.poi.xslf.usermodel.XSLFTable;     case 1: goto $r4 = new org.apache.poi.xslf.usermodel.XSLFObjectShape;     case 2: goto $r2 = new org.apache.poi.xslf.usermodel.XSLFDiagram;     default: goto $r6 = new org.apache.poi.xslf.usermodel.XSLFGraphicFrame; } 
(assert (and (= var1334 2) (and (not (= var1334 1)) (and (not (= var1334 0)) true)))) ; Intersect: Cond: b1 == 2  and Intersect: Negate: Cond: b1 == 1   and Intersect: Negate: Cond: b1 == 0   and Non Conditional   
(define-const var2209 var1166 var1166-init) ; Statement: $r2 = new org.apache.poi.xslf.usermodel.XSLFDiagram 
(assert true)
;(assert (<init>/2139047500 var2209 var1122 var2317)) ; Statement: specialinvoke $r2.<org.apache.poi.xslf.usermodel.XSLFDiagram: void <init>(org.openxmlformats.schemas.presentationml.x2006.main.CTGraphicalObjectFrame,org.apache.poi.xslf.usermodel.XSLFSheet)>(r0, r3) 

(declare-const var2209!1 var1166)
(declare-const var1122!1 var1174)
(declare-const var2317!1 var1570)
 ; Statement: return $r2 
(check-sat)
(get-model)
(get-unsat-core)
; {var2669_getUri/626169278=([org.openxmlformats.schemas.presentationml.x2006.main.CTGraphicalObjectFrame], java.lang.String), hashCode/-467973558=([java.lang.String], int), var1166-init=([], org.apache.poi.xslf.usermodel.XSLFDiagram), <init>/2139047500=([org.apache.poi.xslf.usermodel.XSLFDiagram, org.openxmlformats.schemas.presentationml.x2006.main.CTGraphicalObjectFrame, org.apache.poi.xslf.usermodel.XSLFSheet], void)}
; {var1174=org.openxmlformats.schemas.presentationml.x2006.main.CTGraphicalObjectFrame, var1122=r0, var1570=org.apache.poi.xslf.usermodel.XSLFSheet, var2317=r3, var2669=org.apache.poi.xslf.usermodel.XSLFGraphicFrame, var996=r1, var1349=null_type, var1336=$r7, var1334=b1, var111=$i0, var1166=org.apache.poi.xslf.usermodel.XSLFDiagram, var2209=$r2}
; {org.openxmlformats.schemas.presentationml.x2006.main.CTGraphicalObjectFrame=var1174, r0=var1122, org.apache.poi.xslf.usermodel.XSLFSheet=var1570, r3=var2317, org.apache.poi.xslf.usermodel.XSLFGraphicFrame=var2669, r1=var996, null_type=var1349, $r7=var1336, b1=var1334, $i0=var111, org.apache.poi.xslf.usermodel.XSLFDiagram=var1166, $r2=var2209}
;seq <java.lang.String: int hashCode()>
;cnt {"<java.lang.String: int hashCode()>": 1}
;stmts r0 := @parameter0: org.openxmlformats.schemas.presentationml.x2006.main.CTGraphicalObjectFrame;	r3 := @parameter1: org.apache.poi.xslf.usermodel.XSLFSheet;	r1 = staticinvoke <org.apache.poi.xslf.usermodel.XSLFGraphicFrame: java.lang.String getUri(org.openxmlformats.schemas.presentationml.x2006.main.CTGraphicalObjectFrame)>(r0);	if r1 != null goto $r7 = r1;	$r7 = "";	goto [?= b1 = -1];	b1 = -1;	$i0 = virtualinvoke $r7.<java.lang.String: int hashCode()>();	lookupswitch($i0) {     case -844661481: goto $z2 = virtualinvoke $r7.<java.lang.String: boolean equals(java.lang.Object)>("http://schemas.openxmlformats.org/presentationml/2006/ole");     case -809269002: goto $z1 = virtualinvoke $r7.<java.lang.String: boolean equals(java.lang.Object)>("http://schemas.openxmlformats.org/drawingml/2006/diagram");     case -603062095: goto $z0 = virtualinvoke $r7.<java.lang.String: boolean equals(java.lang.Object)>("http://schemas.openxmlformats.org/drawingml/2006/table");     default: goto tableswitch(b1) {     case 0: goto $r5 = new org.apache.poi.xslf.usermodel.XSLFTable;     case 1: goto $r4 = new org.apache.poi.xslf.usermodel.XSLFObjectShape;     case 2: goto $r2 = new org.apache.poi.xslf.usermodel.XSLFDiagram;     default: goto $r6 = new org.apache.poi.xslf.usermodel.XSLFGraphicFrame; }; };	tableswitch(b1) {     case 0: goto $r5 = new org.apache.poi.xslf.usermodel.XSLFTable;     case 1: goto $r4 = new org.apache.poi.xslf.usermodel.XSLFObjectShape;     case 2: goto $r2 = new org.apache.poi.xslf.usermodel.XSLFDiagram;     default: goto $r6 = new org.apache.poi.xslf.usermodel.XSLFGraphicFrame; };	$r2 = new org.apache.poi.xslf.usermodel.XSLFDiagram;	specialinvoke $r2.<org.apache.poi.xslf.usermodel.XSLFDiagram: void <init>(org.openxmlformats.schemas.presentationml.x2006.main.CTGraphicalObjectFrame,org.apache.poi.xslf.usermodel.XSLFSheet)>(r0, r3);	return $r2
;block_num 5